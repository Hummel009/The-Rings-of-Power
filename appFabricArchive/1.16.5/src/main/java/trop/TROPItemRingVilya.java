...
public class TROPItemRingVilya extends TROPItemRingBase {
	...
	public void appendTooltip(ItemStack itemStack, World world, List<Text> list, TooltipContext tooltipContext) {
		for (StatusEffect statusEffect : new StatusEffect[]{StatusEffects.SPEED, StatusEffects.JUMP_BOOST}) {
	...
	public void inventoryTick(ItemStack itemStack, World world, Entity entity, int i, boolean b) {
		if (entity instanceof LivingEntity) {
			((LivingEntity) entity).addStatusEffect(new StatusEffectInstance(StatusEffects.SPEED, 20, 1));
			((LivingEntity) entity).addStatusEffect(new StatusEffectInstance(StatusEffects.JUMP_BOOST, 20, 1));
		}
	}
