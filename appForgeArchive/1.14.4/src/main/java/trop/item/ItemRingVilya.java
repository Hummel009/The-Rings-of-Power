...
public class ItemRingVilya extends ItemRing {
	...
	public void appendHoverText(ItemStack itemStack, World world, List<ITextComponent> list, ITooltipFlag tooltipFlag) {
		for (Effect effect : new Effect[]{Effects.MOVEMENT_SPEED, Effects.JUMP}) {
	...
	public void inventoryTick(ItemStack itemStack, World world, Entity entity, int slot, boolean selected) {
		if (entity instanceof LivingEntity) {
			((LivingEntity) entity).addEffect(new EffectInstance(Effects.MOVEMENT_SPEED, 20, 1));
			((LivingEntity) entity).addEffect(new EffectInstance(Effects.JUMP, 20, 1));
		}
	}
